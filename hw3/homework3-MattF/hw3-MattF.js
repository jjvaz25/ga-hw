...
function problem(x) {
    if (isNaN(x)) {
        let userError = x;
        console.log(`ERROR: ${userError} is not a number, but it's a start. Let's try again`);
    } else {
        let fiftyTimes = x * 50 + 6;
        console.log(fiftyTimes);
    }
...
function maxOfTwoNumbers(x, y) {
    ...
    let greaterNumber = Math.max(x, y);
...
const maxOfThree = function (x, y, z) {
    ...
    let maxOfXandY = Math.max(x, y);
    let greatestOfThree = Math.max(maxOfXandY, z);
