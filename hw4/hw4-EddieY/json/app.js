...
let copiedgrungeAlbums = JSON.parse(grungeAlbumsJSON);
console.log(copiedgrungeAlbums);
...
console.log(copiedgrungeAlbums.albums);

for (i = 0; i < copiedgrungeAlbums.albums.length; i++) {
  let obj = copiedgrungeAlbums.albums[i];

  for (var key in obj) { 
    if (key === "name") {
      console.log(`Album: ${obj[key]}`)
    } else if (key === "artist") {
      console.log(`Artist: ${obj[key]}`)
    } else if (key === "unitsSold") {
      console.log(`Units sold: ${obj[key]}`)
    }  
  }
};
